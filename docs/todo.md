...
* [x] `splitshot --help` に新コマンド群（`prompts|step1|step2|step3|worktrees|run|integrate|cleanup|tail`）が並び、説明が最新化されている
* [x] 新フロー：`prompts up → step1 spec → step2 design → step3 gen-prompts → worktrees up → run → integrate → cleanup` が**E2Eテスト**で緑
* [x] 旧 `plan` コマンド・旧 `manifest.json`・旧 `plan.stub.json` など v1 系はビルドから除去
* [x] CI（Vitest）グリーン、最低限の README 更新

---

# 0. E2E 骨格（失敗から始める）

* [x] **RED**: `tests/e2e/new-flow.e2e.test.ts`
...
* [x] **GREEN**: 各章で作る最小実装がそろった段階でこのテストを徐々に通す
...
* [x] **RED**: `tests/cli.step1.spec.e2e.test.ts`
...
* [x] **RED**: `tests/cli.step2.design.e2e.test.ts`

  * 実行で `docs/todo/agent-a.md` など複数ファイルと `docs/interface.md` を生成
  * `docs.index` 更新
* [x] **GREEN**: `src/cli/step2.ts` 実装（Step1 と同パターン、プロンプトは `/split`）
...
* [x] **RED**: `tests/cli.step3.gen-prompts.test.ts`
...
* [x] **GREEN**: `src/cli/step3.ts` & `src/core/todoParser.ts`
...
* [x] **GREEN**: `src/core/worktrees.ts`, `src/cli/worktrees.ts`

  * `git worktree add/remove`, `branch` 操作のラッパー
  * ルート外パスの安全性チェック
* [x] **REFACTOR**: Git ラッパーとメッセージ共通化

---

# 8. `run` の v2 対応（manifest v3 準拠）

* [x] **RED**: `tests/run.v2.e2e.test.ts`
...
* [x] **GREEN**: `src/cli/run.ts` 改修（読み物を v3 に切り替え）
...
* [x] **RED**: `tests/cli.cleanup.test.ts`

  * マージ済みブランチのみ削除、未マージはデフォルト拒否
* [x] **GREEN**: `src/cli/cleanup.ts`（`worktrees.down` を内部利用）
* [ ] **REFACTOR**: エラー文言統一
...
* [x] **RED**: `tests/cli.tail.v2.test.ts`

  * v3 の `.runs/latest.json` と `events.ndjson` を辿れる
  * `--run`, `--type` フィルタ継続
* [x] **GREEN**: `src/cli/tail.ts`（v3 パス解決へ差し替え）
...
* [x] **RED**: `tests/legacy.removed.test.ts`

  * `splitshot plan` 実行で**エラー**または**非存在**（後方互換なし）
* [x] **GREEN**: `src/cli/plan.ts` と関連参照（`schemas/plan.ts` 等）を削除 or `deprecated/` へ隔離しビルド対象外
...
* [x] **RED**: `tests/help.output.test.ts`

  * `splitshot --help` と各サブコマンド `--help` の出力が新仕様に整合
* [x] **GREEN**: コマンド説明・例を更新
* [x] **REFACTOR**: README を「新フロー」へ全面差し替え
